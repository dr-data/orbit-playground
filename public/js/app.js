...
function syncReadouts() {
  ...
  if (activeLaunch === "geo" || activePreset === "geo") {
    if (frameMode === "space") {
      coachEl.textContent = "Space frame: Earth spins once a day. GEO keeps pace, so it hovers over the gold mountain.";
    } else {
      coachEl.textContent = "Earth frame: the globe is frozen. GEO still circles once a day in this inertial view.";
    }
  } else if (activeLaunch === "ngeo") {
    coachEl.textContent = frameMode === "space"
      ? "Same height as GEO, but thrown the other way — it will not hover over the mountain."
      : "Retrograde at GEO height. Earth is frozen, so you just see it go the opposite way.";
  } else if (frameMode === "earth" && cls.kind === "circular") {
    coachEl.textContent = `${cls.coach} Earth is drawn still (Earth frame).`;
  }
